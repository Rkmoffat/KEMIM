...
ArtVisJ2Stress::validParams()
{
  ...
  params.addRequiredParam<Real>("lipit", "lipit");
  return params;
}

ArtVisJ2Stress::ArtVisJ2Stress(
...
void
ArtVisJ2Stress::initialSetup()
{
  _flow_stress_material = &getMaterial("flow_stress_material");

  // Enforce isotropic elastic tensor
  if (!hasGuaranteedMaterialProperty(_elasticity_tensor_name, Guarantee::ISOTROPIC))
    mooseError("ArtVisJ2Stress requires an isotropic elasticity tensor");
}

void
ArtVisJ2Stress::initQpStatefulProperties()
{
...
void
ArtVisJ2Stress::computeQpPK1Stress()
{
  ...
  RankTwoTensor tau = _pressure_total[_qp] * I + s;
...
Real
ArtVisJ2Stress::computeReferenceResidual(const Real & effective_trial_stress,
                                                              const Real & scalar)
{
  const Real G = ElasticityTensorTools::getIsotropicShearModulus(_elasticity_tensor[_qp]);
...
Real
ArtVisJ2Stress::computeResidual(const Real & effective_trial_stress,
                                                     const Real & scalar)
{
  const Real G = ElasticityTensorTools::getIsotropicShearModulus(_elasticity_tensor[_qp]);
...
Real
ArtVisJ2Stress::computeDerivative(const Real & /*effective_trial_stress*/,
                                                       const Real & scalar)
{
  const Real G = ElasticityTensorTools::getIsotropicShearModulus(_elasticity_tensor[_qp]);
...
void
ArtVisJ2Stress::preStep(const Real & scalar, const Real & R, const Real & J)
{
  if (!_fe_problem.currentlyComputingJacobian())
    return;
